...
# Linker flags
LDFLAGS += -L/home/lwang323/ms-bfs/boost/stage/lib  # Add the Boost library path
LDLIBS += -lboost_iostreams -lboost_system -lboost_thread -lboost_timer -lboost_chrono -pthread -lrt
...
EXECUTABLE_BENCHER_SIMPLE=runBencherSimple
...
all: $(EXEC_EXECUTABLE) $(EXECUTABLE_BENCHER) $(EXECUTABLE_BENCHER_SIMPLE)
	@rm -f $(CORE_DEPS)
...
clean:
	-rm $(EXECUTABLE_FAST) $(EXECUTABLE_DEBUG) $(EXECUTABLE_BENCH_PROFILE) $(EXECUTABLE_BENCH) $(EXECUTABLE_BENCHER) $(EXECUTABLE_BENCHER_SIMPLE)
	-rm $(CORE_OBJECTS) $(RELEASE_OBJECTS) *.o
	-rm $(CORE_DEPS)
	-rm *.gcda util/*.gdca

$(EXECUTABLE_BENCH): $(ALL_SOURCES)
	$(CC) $(RELEASE_CFLAGS) $(CORE_SOURCES) -fprofile-generate $< -o $(EXECUTABLE_BENCH_PROFILE) $(LD_FLAGS) $(LDLIBS)
	./$(EXECUTABLE_BENCH_PROFILE) $(10K_QUERIES) 1 1
	$(CC) $(RELEASE_CFLAGS) $(CORE_SOURCES) -fprofile-use $< -o $(EXECUTABLE_BENCH) $(LD_FLAGS) $(LDLIBS)
	-rm *.gcda util/*.gcda
	-rm $(EXECUTABLE_BENCH_PROFILE)

$(EXECUTABLE_BENCH_THREADS): benchThreads.release.o $(RELEASE_OBJECTS) $(BOOST_LIBS)
	@rm -f $(CORE_DEPS)
	$(CC) benchThreads.release.o $(RELEASE_OBJECTS) $(LDFLAGS) -o $@ $(LD_FLAGS) $(LDLIBS)

$(EXECUTABLE_BENCHER): runBencher.release.o $(RELEASE_OBJECTS) $(BOOST_LIBS)
	@rm -f $(CORE_DEPS)
	$(CC) runBencher.release.o $(RELEASE_OBJECTS) $(LDFLAGS) -o $@ $(LD_FLAGS) $(LDLIBS)
$(EXECUTABLE_BENCHER_SIMPLE): runBencherSimple.release.o $(RELEASE_OBJECTS) $(BOOST_LIBS)
	@rm -f $(CORE_DEPS)
	$(CC) runBencherSimple.release.o $(RELEASE_OBJECTS) $(LDFLAGS) -o $@ $(LD_FLAGS) $(LDLIBS)

$(EXECUTABLE_BENCH_VARIANTS): benchVariants.release.o $(RELEASE_OBJECTS) $(BOOST_LIBS)
	@rm -f $(CORE_DEPS)
	$(CC) benchVariants.release.o $(RELEASE_OBJECTS) $(LDFLAGS) -o $@ $(LD_FLAGS) $(LDLIBS)

ifndef DEBUG
$(EXEC_EXECUTABLE): main.release.o $(RELEASE_OBJECTS) $(BOOST_LIBS)
	@rm -f $(CORE_DEPS)
	$(CC) main.release.o $(RELEASE_OBJECTS) $(LDFLAGS) -o $@ $(LD_FLAGS) $(LDLIBS)
else
$(EXEC_EXECUTABLE): main.o $(CORE_OBJECTS)
	@rm -f $(CORE_DEPS)
	$(CC) main.o $(CORE_OBJECTS) -o $@ $(LD_FLAGS) $(LDLIBS)
endif

%.release.o: %.cpp
	$(CC) $(RELEASE_CFLAGS) $(LDFLAGS) -c $< -o $@ $(LDLIBS)

.cpp.o:
	$(CC) $(CFLAGS) -DTRACE -c $< -o $@ $(LDLIBS)


%.depends: %.cpp
	@$(CC) -M $(CFLAGS) -c $< > $@ $(LDLIBS)
